...
def generate_token_dict(vocab):
    ...
    for i in range(len(vocab)):
        # vocab[i] = s => create: token_dict{ "s":i}
...
def prepocess_input_sequence(
    ...
    for i in range(len(input_str)):
        if input_str[i].isdigit():
            # if is digit: break down to "one digie per entry in output list"
            for char in input_str[i]:
                out.append(token_dict[char])
        else:
            out.append(token_dict[input_str[i]])
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_single(
    ...
    # print("K, M ", K, M)
    weights = torch.zeros((K,Kk), dtype=query.dtype, device=query.device)
    for i in range(K):
        for j in range(Kk):
            weights[i,j] = torch.dot(query[i,:], key[j,:]).div(M**0.5)
...
def scaled_dot_product_two_loop_batch(
    ...
    # print("N, K, M ", N, K, M)
                        # (N , K_query, K_key)
    weights = torch.zeros((N, K,Kk), dtype=query.dtype, device=query.device)    

    # Weights: N, (K, K)
    for b in range(N):
        # for each row vector of K_query
        for i in range(K):
                                # (1, M) @ (M, K_key) => (1, Kkey)
            weights[b, i,:] = query[b,i,:].reshape(1,-1) @ (key.transpose(1,2)[b,:,:]) / (M**0.5)
...
def scaled_dot_product_no_loop_batch(
    ...
    weights = torch.zeros((N, K,Kk), dtype=query.dtype, device=query.device)    
                ...
                # (N, K, M) @ (N, M, Kk) => (N, K, Kk)
    weights = torch.matmul(query,(key.transpose(1,2))) / (M**0.5)
    ...
    if mask is not None:
        ...
        ##########################################################################
        if len(mask.shape) != len(weights.shape):
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        ##########################################################################
        # print("dim_in ", dim_in, " dim_q ",dim_q, " dim_v ", dim_v)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        # SelfAttention.                                                         #
        ##########################################################################
        # Create a list of SelfAttention
        self.attention_layers = nn.ModuleList()
        for i in range(num_heads):
            # print("i ", i, " singleAttention(dim_in, dim_q, dim_v )", dim_in, dim_out, dim_out)
    ...
    def forward(
        ...
        for i in range(len(self.attention_layers)):
            y_ = self.attention_layers[i](query, key, value, mask)  
            y.append(y_)
        # Now: each y_ (2,4,8)
...
class LayerNormalization(nn.Module):
    def __init__(self, emb_dim: int, epsilon: float = 1e-10):
        ...
        ##########################################################################
        # self.gamma = torch.ones(emb_dim) # M
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        torch.nn.init.uniform_(self.MLP1.weight, a=-cq, b=cq)
        torch.nn.init.constant_(self.MLP1.bias, 0)

        ck = (6/(inp_dim + hidden_dim_feedforward))**0.5
        torch.nn.init.uniform_(self.MLP2.weight, a=-ck, b=ck)
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        self.multihead_attention_block = MultiHeadAttention(num_heads, emb_dim, emb_dim//num_heads)
    ...
    #     - feedforward - out3 - layer_norm(out3 + out2) - dropout - out
        ...
        out1 = self.multihead_attention_block(x, x, x)
        # print("out1.shape" ,out1.shape)
        out2 = self.dropout(self.LN1(out1+x))
        out3 = self.feedforward(out2)
        y = self.dropout(self.LN2(out3+out2))
...
def get_subsequent_mask(seq):
    ...
    ###############################################################################
    # expected: (4,3,3)
    # KEY: Make upper triangular True. => Fill False from col 0(n,k,:j) to col K-1 (n,k,:)
    # ([[False,  True,  True],
      # [False, False,  True],
      # [False, False, False]])

    N, K = seq.shape
    mask = torch.full((N,K,K),fill_value=True, dtype=torch.bool, device=seq.device)
    for n in range(N):
        # For each row:
        for k in range(K):
            mask[n,k,:k+1] = False 
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        # layer_norm(out5 + out4) - dropout - out       
                        ...
                        # query, key, value
        # print("\nIn function Decoder_block.forward:")
        # print("dec_inp.shape ", dec_inp.shape)
        out1 = self.attention_self(dec_inp, dec_inp, dec_inp, mask)
        # print("out1.shape ", out1.shape)
        out1 = self.norm1(out1 + dec_inp)
        # print("After norm, out1.shape ", out1.shape)
        out2 = self.dropout(out1)              # Should be (2, 2, 4)

        # print("enc_inp.shape ", enc_inp.shape)
        # print("out2.shape ", out2.shape)

        # print("-- attention_cross.forward:")
        # print("q, k, v .shape: ", out2.shape, enc_inp.shape, enc_inp.shape)
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    # print(y_.repeat(M,1).transpose(1,0).reshape(1,K,M))
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    # print("K ", K, " M ",M)
    # print("M//2 ", M//2)
    # print("floor(M/2) ", floor(M/2))
    # print("ceil(M/2) ", ceil(M/2))
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        # print("a_emb_inp.shape ", a_emb_inp.shape)
        # print("ans_b.shape ", ans_b.shape)
        # print("q_emb.shape ", q_emb.shape)
        # print("ques_pos.shape ", ques_pos.shape)
        mask = get_subsequent_mask(ans_b[:,:-1])
                        # Target_sequence, encoder_output, mask
