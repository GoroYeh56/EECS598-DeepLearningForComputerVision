...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    # length = int((stop - start) / 10)

    if start%10 ==0:
      start_val = start
    else:
      start_val = (start//10 +1 ) * 10

    stop_val = (stop//10 +1 )*10
    x = torch.arange(start_val, stop_val, 10, dtype=torch.float64)  #[start, end) 

    # length = int(stop/10)

    # print("\nnumber of items: ", length)
    # x = torch.zeros((length,), dtype=torch.float64)

    # index = 0
    # for i in range(start, stop+1): # since Exclusive
    #   if i%10 ==0 and i>0:
    #     x[index] = i
    #     index += 1
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    ones = torch.ones(len(x), C)
    id0 = torch.arange(len(x))
    id1 = x
    y[id0, id1] = 1 # make these position to be '1'
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    y = x.view(2,3,4).permute(1,0,2).reshape(3,-1)
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    id0 = torch.arange(y.shape[0])
    id1 = torch.argmin(y, dim=1)
    y[id0, id1] = 0
    ...
    #   idx = y[i,:].argmin()
    #   print('min idx: ',idx.item())
    #   print('value: ', y[i, idx].item())
    #   y[i, idx] = 0
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    mu = y.sum(dim=0) / num_rows # mu is a vector of 'num_cols' values (3 columns here)
    print('mu: ', mu)
    tmp = y
    print('tmp-mu: ', (tmp-mu))
    sigma = torch.sqrt( ((tmp - mu)**2 ).sum(dim=0)  / (num_rows-1))
    # print('sigma: ', sigma)
    y = (y-mu)/sigma   

    # for j in range(num_cols):
    #     mu_j = y[:,j].sum() / num_cols
        
    #     tmp = y[:,j]
    #     sigma_j = torch.sqrt(torch.sum((tmp - mu_j)**2 ) / (num_rows-1))
        
    #     y[:,j] = (y[:,j]-mu_j)/sigma_j
        # print('col ',j, ' mu_j: ', mu_j, ' sigma: ',sigma_j, 'y[:,j] : ', y[:,j])
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    uniques=torch.unique( x, sorted=True, return_inverse=False)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
