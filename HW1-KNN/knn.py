...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      train_img = x_train[i].view(1,-1)
      for j in range(num_test):
        test_img = x_test[j].view(1,-1)
        # length = test_img.shape[1] #3072
        # print('length: ', train_img.shape[1])
        # print('length: ', test_img.shape[1])
        # if i==0 and j==0:
          # print('train_img: ', train_img)
          # print('test_img: ', test_img)
          # print(train_img - test_img)
          # print( ((train_img - test_img)**2).shape[0] )
        dists[i, j] = ((train_img - test_img)**2).sum(dim=1)
        # if i==0  and j==0:
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      train_img = x_train[i].view(1,-1)
      test_img = x_test.reshape(num_test, -1)
      dists[i] = ((train_img - test_img)**2).sum(dim=1)
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    train_img = x_train.reshape(num_train,-1)
    test_img = x_test.reshape(num_test, -1)
    
    ######## Q: Why this? 
    # (a-b)^2 = a^2 + b^2 - 2*a*b
    dists =  train_img.pow(2).sum(dim=1).view(-1,1) + test_img.pow(2).sum(dim=1).view(1,-1)  - 2*torch.mm(train_img, test_img.t())
    # print( 2*torch.mm(train_img, test_img.t()).shape )
    print(dists.shape)
    # dists = ((train_img - test_img.transpose(0,1))**2).sum(dim=2)
    # print("shape of dists ",dists.shape) # should be 100x100
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for j in range(num_test):
      ...
      # print('dist[:,j] ', dists[:,j])
      # Pick top k nearest neighbors
      res, indices = torch.topk(dists[:,j], k, dim=0, largest=False)
      # print('nn indices: ', indices)
      # print('Labels: ', y_train[indices])
      # k nearest neighbors' label (ground truth) 
      num_classes = max(y_train)+1
      classes_cnt = torch.zeros(num_classes) # Since only 12 classes
      
      # Pick the majority of these classes
      for idx in indices: # means train[idx]
        classes_cnt[y_train[idx]] += 1
      # print('classes_cnt: ', classes_cnt)
      # Assign this y_pred to the class 
      y_pred[j] = torch.argmax(classes_cnt)
      # print('predicted y: ', y_pred[j])
      # print('\n')
...
def knn_cross_validate(
    ...
    print('x_train_folds length: ', len(x_train_folds))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    ...
    for k in k_choices:
      #k : current k to perform 'num_folds' knn.
      accuracies = []
      for it in range(num_folds): # it: 0,1,2,3,4
        # run num_folds times KNN !
        if it==0:
          x_train = torch.cat(x_train_folds[0:num_folds-1], 0)
        elif it==num_folds-1:
          x_train = torch.cat(x_train_folds[1:num_folds], 0)
        else:
          first = torch.cat(x_train_folds[0:num_folds-it-1])
          second = torch.cat(x_train_folds[num_folds-it:num_folds])
          x_train = torch.cat( (first, second) , 0)

        x_test =  x_train_folds[num_folds-it-1]


        if it==0:
          y_train = torch.cat(y_train_folds[0:num_folds-1], 0)
        elif it==num_folds-1:
          y_train = torch.cat(y_train_folds[1:num_folds], 0)
        else: #1, 2, 3
          first = torch.cat(y_train_folds[0:num_folds-it-1])
          second = torch.cat(y_train_folds[num_folds-it:num_folds])
          y_train = torch.cat( (first, second) , 0)

        y_test =  y_train_folds[num_folds-it-1]        
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accs in sorted(k_to_accuracies.items()): 
      ...
      if mean_acc > max_acc:
        max_acc = mean_acc
        best_k = k
    # best_k = k_to_accuracies.keys()[np.argmax(ave_acc)]
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
